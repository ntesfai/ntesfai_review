...
public class SortDemoData  {
        ...
        NoSuchElementException, NegativeArraySizeException{
                ...
                int arrayLen; // will be the length of myArray        
                if (inputTokens.hasMoreTokens()) {
                        arrayLen = inputTokens.countTokens(); 
                } else {
                        throw new NoSuchElementException();
                }

                // define the array of the right length arrayLen    
                if (arrayLen == 1) { 

                        // interpret the next number as number of entries and 
                        // generate a random vector with entries 1 ... arrayLen
                        str = inputTokens.nextToken(); 
                        arrayLen =  Integer.valueOf(str).intValue();
                        // may throw NegativeArraySizeException
                        myArray = new Item[arrayLen];
                        boolean[] tempArray = new boolean[arrayLen + 1];
                        RandomNumber randGen = new RandomNumber();
                        int randNr;
                        for (int i = 0; i < arrayLen; i++) {
                                do 
                                        randNr = randGen.nextIntRand(1, arrayLen);
                                while (tempArray[randNr]);                  
                                tempArray[randNr] = true;
                                myArray[i] = new Item(randNr);
                        }
                } else {           
                        Item [] array = new Item[arrayLen];
                        int count =0;
                        // read the arrayLen numbers into the array
                        for (int i = 0; i < arrayLen; i++) {
                        ...
                        myArray = new Item[arrayLen-count];
                        myArray = Arrays.copyOfRange(array, 0, arrayLen-count);
